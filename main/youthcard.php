...
<a  href="youthinfo.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM youth WHERE registered = 'card'");
				$result->execute();
			...
			Total Number of Registered Youth Members:  <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $rowcount;?>]</font>
			</div>
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			// Database connection parameters
				...
				$selectedCountry = isset($_GET['registered']) ? $_GET['registered'] : 'card';

				// Query to retrieve members from the selected country
				$query = "SELECT * FROM youth WHERE registered = ?";
			...
			<td><a title="Click to view the Member" href="youthactive.php?id=<?php echo $row['username']; ?>"><button class="btn btn-success btn-mini"><i class="icon-search"></i> Activate</button> </a>
			<a  title="Click to edit the Member" href="youthpend.php?id=<?php echo $row['username']; ?>"><button class="btn btn-warning btn-mini"><i class="icon-edit"></i> Pending</button> </a>
			<a  href="#" id="<?php echo $row['username']; ?>" class="delbutton" title="Click To Delete"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Delete</button></a></td>
			</tr>
			<?php
				}
